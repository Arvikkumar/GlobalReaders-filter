...
async def pm_next_page(bot, query):
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⬅️ 𝘽𝙖𝙘𝙠", callback_data=f"pmnext_{req}_{key}_{off_set}"),
        ...
        btn.append(
            [InlineKeyboardButton(f"❄️ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
             InlineKeyboardButton("𝙉𝙚𝙭𝙩 ➡️", callback_data=f"pmnext_{req}_{key}_{n_offset}")])
    else:
        btn.append([
            InlineKeyboardButton("⬅️ 𝘽𝙖𝙘𝙠", callback_data=f"pmnext_{req}_{key}_{off_set}"),
            InlineKeyboardButton(f"❄️ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages"),
...
async def pm_AutoFilter(client, msg, pmspoll=False):    
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"❄️ 𝙋𝙖𝙜𝙚 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
            InlineKeyboardButton(text="𝙉𝙚𝙭𝙩 ➡️", callback_data=f"pmnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="❄️ 𝙋𝙖𝙜𝙚 1/1", callback_data="pages")]
        )
    if PM_IMDB:
